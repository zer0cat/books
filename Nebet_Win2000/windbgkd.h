...
{
    l64->Flink = (DWORDLONG)(LONGLONG)(LONG)l32->Flink;
    l64->Blink = (DWORDLONG)(LONGLONG)(LONG)l32->Blink;
}
...
typedef struct _PLABEL_DESCRIPTOR {
   DWORDLONG EntryPoint;
   DWORDLONG GlobalPointer;
...
typedef struct _DBGKD_GET_VERSION {
    ...
    DWORD_PTR   KiCallUserMode;             // kernel routine

    //
    // Address of the usermode entry point for callbacks.
    //

    DWORD_PTR   KeUserCallbackDispatcher;   // address in ntdll
    ...
    DWORD_PTR   BreakpointWithStatus;       // address of breakpoint
